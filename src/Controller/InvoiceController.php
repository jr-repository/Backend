...
class InvoiceController
{
    ...
    public function save()
    {
        ...
        if (empty($input['customerNo']) || empty($input['items'])) {
            Response::json(['message' => 'Data Customer dan Item wajib diisi'], 400, false);
        }
    ...
    public function approve()
    {
        ...
        if (empty($id)) {
            Response::json(['message' => 'Invalid Request: ID Kosong'], 400, false);
        }
    ...
    public function pay()
    {
        ...
        if (empty($id)) {
            Response::json(['message' => 'ID Invoice wajib diisi'], 400, false);
        }
        if (empty($files['name'][0])) {
            Response::json(['message' => 'Wajib mengupload minimal 1 file bukti pembayaran'], 400, false);
        }

        $result = $this->service->processPayment($id, $paidAmount, $userId, $files);
    ...
    {
        $data = $this->service->getMasterCustomers();
        if ($data) {
    ...
    {
        $data = $this->service->getMasterItems();
        if ($data) {
            header('Content-Type: application/json');
            echo json_encode($data);
            exit;
